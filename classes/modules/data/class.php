...
class data extends def_module {
	...
	private function renderRelation(umiObjectProperty &$property, $template, $showNull = false, $isRandom = false) {
		...
		if($property->getIsMultiple() === false) {
			...
			$arrayBlock['value'] = umiObjectsCollection::getInstance()->getObject($value)->getName();
			return self::parseTemplate($tpl, $arrayBlock);
		} else {
			...
			for($i = 0; $i < $sz; $i++) {
				$arrayItem = array(
					'object_id' =>  $value[$i],
					'value' => umiObjectsCollection::getInstance()->getObject($value[$i])->getName(),
				);
				$arrayItem['quant'] = ($sz != ($i + 1)) ? $tpl_quant : "";
		...
		foreach($value as $info) {
			...
			if($objectId) {
				if($object = selector::get('object')->id($objectId)) {
					$itemArray['object-id'] = $object->id;
					$itemArray['object-name'] = $object->name;
				}
			}

			if($elementId) {
				if($element = selector::get('element')->id($elementId)) {
					$itemArray['element-id'] = $element->id;
					$itemArray['element-name'] = $element->name;
