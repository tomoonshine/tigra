...
<xsl:stylesheet	version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="payment" mode="payment_new">
	...
	<xsl:template match="item" mode="payment">
		<div class="control-group row-fluid control-group-{@id} box-content">
			<div class="span5"><xsl:value-of select="@name" /></div>
			 <div class="span7">                                                    
				<label>
					<xsl:if test="@type-name = 'receipt'">
						<script>
	...
	<xsl:template match="purchasing[@stage = 'payment'][@step = 'kupivkredit'][@action = 'widget']">
		...
		<xsl:apply-templates select="//steps" />
		<h4>В процессе</h4>
		<p>Оформление заявки на получение кредита.</p>
	</xsl:template>

	<xsl:template match="purchasing[@stage = 'payment'][@step = 'kupivkredit'][@action = 'declined']">
		<xsl:apply-templates select="//steps" />
		<h4>Ошибка</h4>
		<p>Заявка на кредит была отклонена.</p>
		<p><a href="/emarket/purchase/payment">К выбору оплаты</a></p>
	</xsl:template>
