...
<xsl:output method="html" encoding="utf-8" indent="yes" doctype-public="-//W3C//DTD XHTML 1.0 Strict//EN" doctype-system="http://www.w3.org/TR/xhtml1/DTD/xhtml1-strict.dtd" />
	...
	<xsl:variable name="module" select="/result/udata/@module" />
	<xsl:variable name="method" select="/result/udata/@method" />
	...
	<xsl:template match="/">
		<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="ru" lang="ru">
			<head>
				...
				<script type="text/javascript" src="/js/jquery/jquery.js" charset="utf-8"></script>
				<script type="text/javascript" src="/templates/demodizzy/js/__common.js" charset="utf-8"></script>
			</head>
			<body>
				<div class="container">
				...
				<div class="container">
					<div class="links">
						<xsl:apply-templates select="document(concat('uobject://', /result/udata/@socialId))//property[@name='iframe_pages']/value/page" mode="menu"/>
					</div>
				</div>
				<xsl:if test="result/udata/@module = 'catalog'">
					<div class="container">
				...
				<div class="container">
					...
					<div class="content">
						...
						<xsl:apply-templates select="result/udata" />
						<div class="clear" />
					</div>
				</div>
			</body>
	...
	<xsl:template match="udata[@module = 'content'][@method = 'content']">
		<xsl:value-of select=".//property[@name = 'content']/value" disable-output-escaping="yes" />
	</xsl:template>

	<xsl:template match="udata[@module = 'content'][@method = 'content'][page/@is-default]">
		<xsl:apply-templates select="document('udata://catalog/getObjectsList//shop/9//10/')/udata" />
	</xsl:template>

	<xsl:template match="udata[@module = 'catalog'][@method = 'category']">
	...
	<xsl:template match="page" mode="objects_list">
		...
		<div class="object_item">
			...
			<div class="price">
				<a id="add_basket_{@id}" class="button basket_list_ options_{$is_options}" href="{$lang-prefix}/emarket/basket/put/element/{@id}/">
	...
	<xsl:template match="udata[@module = 'catalog'][@method = 'object']">
	...
	<xsl:template match="udata[@method = 'cart']">
		<xsl:apply-templates select="document('udata://emarket/cart')/udata" mode="cart" />
	...
	<xsl:template match="udata[@module = 'emarket' and @method = 'cart']" mode="basket">
		<div class="basket_info">
			<div>
				<a href="{$lang-prefix}{$contact-prefix}/emarket/cart/">Корзина</a>
				<span class="basket_info_summary">
					<xsl:apply-templates select="summary" mode="basket" />
				</span>
			</div>
	...
	<xsl:template match="summary[amount &gt; 0]" mode="basket">
		...
		<xsl:text> шт товаров на сумму </xsl:text>
		<xsl:value-of select="price/@prefix" /><xsl:text> </xsl:text><xsl:apply-templates select="price/actual" /> <xsl:value-of select="price/@suffix" />
	</xsl:template>

	<xsl:template match="udata[@method = 'cart']" mode="cart">
		<xsl:text>В корзине нет ни одного товара</xsl:text>
	</xsl:template>

	<xsl:template match="udata[@method = 'cart'][count(items/item) &gt; 0]" mode="cart">
		<div class="basket">
			<table class="table">
				<thead>
					<tr>
			...
			<div>
				<a href="{$lang-prefix}{$contact-prefix}/emarket/basket/remove_all/" class="button big basket_remove_all">
	...
	<xsl:template match="udata[@method = 'purchase']">
		<xsl:apply-templates select="document('udata://emarket/purchase/')/udata/purchasing" />
	</xsl:template>
	...
	<xsl:template match="udata[@method = 'ordersList']">
		<xsl:apply-templates select="document('udata://emarket/ordersList')/udata" mode="orderList"/>
	</xsl:template>

	<xsl:template match="udata[@method = 'ordersList']" mode="orderList">
	...
	<xsl:template match="udata[@module = 'news'][@method = 'rubric']">
		<div >
	...
	<xsl:template match="udata[@module = 'news'][@method = 'item']">
		<div >
			<xsl:value-of select=".//property[@name = 'content']/value" disable-output-escaping="yes" />
		</div>
