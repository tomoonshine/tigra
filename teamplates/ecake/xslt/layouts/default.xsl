	...
	<xsl:template match="/" mode="layout">
		...
		<html dir="ltr" lang="en" class="no-js">
			<head>
				...
				<!-- <link rel="stylesheet" type="text/css" href="{$template-resources}plugin/ecake-tinymse/ecake.plugin.css" /> -->
				<!-- END CSS -->
				
				<!-- <xsl:value-of select="document('udata://system/includeQuickEditJs')/udata" disable-output-escaping="yes" /> -->
				<xsl:value-of select="document('udata://content/includeQuickEditJs')/udata" disable-output-escaping="yes" />
			...
			<body class="">
							...
							<div class="pull-center text-center">
								<a href="/" title="&larr; На главную">
									<xsl:if test="$settings_site[@name='logo_site']">
										<img src="{$settings_site[@name='logo_site']/text()}" height="28"><xsl:attribute name="alt"><xsl:choose><xsl:when test="$settings_site[@name='alt_logo_site']"><xsl:value-of select="$settings_site[@name='alt_logo_site']/text()" /></xsl:when><xsl:otherwise>eCake</xsl:otherwise></xsl:choose></xsl:attribute></img>
									</xsl:if>
								</a>
							</div>
						</div>
				...
				<div class="wrapper">
								...
								<div class="row">
									<div class="span3">							
										<div class="logo">
											<a href="/" title="&larr; На главную">
											    <xsl:if test="$settings_site[@name='logo_site']">
												    <img src="{$settings_site[@name='logo_site']/text()}"><xsl:attribute name="alt"><xsl:choose><xsl:when test="$settings_site[@name='alt_logo_site']"><xsl:value-of select="$settings_site[@name='alt_logo_site']/text()" /></xsl:when><xsl:otherwise>eCake</xsl:otherwise></xsl:choose></xsl:attribute></img>
												</xsl:if>
											</a>
										</div>
									</div>
									<div class="span5">							
										<div class="phone-header">
											<xsl:value-of select="$settings_contact[@name='phone']" /> - <a href="{document('upage://304')/udata/page/@link}#callback" role="button" data-toggle="modal" tabindex="-1" >Заказать звонок</a>
										</div>
										<div class="address"><xsl:value-of select="$settings_contact[@name='address']" /><xsl:value-of select="$settings_contact[@name='mode-works']" /></div>
									</div>
									<div class="span4">
										<div class="row-fluid">
											<div class="span12">
					...
					<div class="footer hidden-phone">
						<div class="container">
							<div class="row">	
								<div class="span2">
									<xsl:apply-templates select="document('udata://menu/draw/762')/udata" mode="bottom_menu">
										<xsl:with-param name="block-title" select="'Магазин'" />
										<xsl:with-param name="block-class" select="'support'" />
									</xsl:apply-templates>
								</div>
								<div class="span2">
									<xsl:apply-templates select="document('udata://menu/draw/763')/udata" mode="bottom_menu">
										<xsl:with-param name="block-title" select="'Каталог'" />
										<xsl:with-param name="block-class" select="'categories'" />
									</xsl:apply-templates>

									<hr />

									<!-- Pay with confidence -->
									<xsl:apply-templates select="$settings_site[@name='footer_image']" mode="footer-pay" />
									<!-- End class="confidence" -->
								</div>

								<div class="span4">
								    <xsl:apply-templates select="document('udata://news/lastlist/37/notemplate/3/1/?extProps=publish_date')/udata" mode="footer-stock" />
						...
						<div class="container">
							<div class="row">
								...
								<div class="span4 text-right hidden-phone">
									<p>Сделано на <a href="http://ecakes.ru/" target="_blank">"eCakes.ru"</a></p>
								</div>
							</div>
						</div>
					</div>
				...
				<script type="text/javascript" src="{$template-resources}js/jquery.ecake.js"></script>
				<!-- <script type="text/javascript" src="{$template-resources}plugin/ecake-tinymse/jquery.ecake.plugin.js"></script> -->
				<!-- END JAVASCRIPTS -->
